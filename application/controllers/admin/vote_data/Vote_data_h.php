...
class Vote_data_h extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($page == 'index') {
            ...
            $sql = 
                ...
                FROM vote_data_t a 
                WHERE a.id_vote_data_h = '$id' 
                    AND a.is_active = 1";

            // print_r($this->RawModel->sqlRaw($sql)->row());
            // exit();

            $data = [
                'title' => $vote_data_h->nama,
                'vote_data_h' => $vote_data_h,
                'total_vote_voters' => $this->RawModel->sqlRaw($sql)->row(),
                'content' => 'admin/vote_data/detail',
                'cropper' => 'components/cropper',
                'aspect' => '3/4'
            ];
    ...
    {
        if (isset($_GET['id'])) {
            echo json_encode(['data' => $this->Vote_data_hModel->findBy(['id' => $_GET['id']])->row()]);
        } else {
